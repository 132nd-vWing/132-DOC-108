...
\narrow{%
  ...
  \begin{orderedlist}
    ...
    \item Nav lights per SOP (see \fullref{sec:procedures-lighting})
    ...
    \item Continue start-up from 108\th checklists

    \item Perform swing(/alpha)-checks on Flight internal comms

    \item Perform Aft Radio comms check on ATC frequency

    \item Check in with ATC for taxi clearance
...
\label{sec:procedures-lighting}

\subsubsection{CV Ops}

Please refer to \href
  {https://cloud.132virtualwing.org/s/wSMBb66EEEJztNW}
  {132\nd Carrier Operations 132-TTP-19 Section 3.1.1}

\subsubsection{Airfield Day and Night}

\begin{itemize}
  \item Navigation lights on Flash during startup.

  \item Set to steady prior to Taxiing.

  \item Anti-collision lights off on the ground and enabled immediately prior
    to entering the runway.

  \item Taxi lights off when stationary and enabled immediately prior and
    during taxi, takeoff and landing.

  \item After landing and vacating the runway, the Anti-Collision lights must
    be turned OFF. Verify taxi lights are on.

  \item These items are in accordance to the wing wide lighting policy
    contained in the \href
      {https://cloud.132virtualwing.org/s/sbtfjqD4oQiXHYg}
      {132\nd Multiplayer} \href
      {https://cloud.132virtualwing.org/s/sbtfjqD4oQiXHYg}
      {Instructions}
...
For kneeboard reference, see \fullref{sec:appendix-lights}
...
\sidebyside{0.75}{%
  ...
  In general, all Pilots should avoid Afterburner and consider it an emergency
  or combat only tactic. Additionally, care should be taken to use conservative
  fuel flow as soon as is practically possible and plan altitude and missions
  accordingly.

  The default loadout by 108\th will always be to include extra fuel tanks
  unless specified and retain them.

  The AoA indexer has two marks. The  {\color{blue}Blue} mark at 5 units for
  "max power climb" and {\color{red}Red} mark for "cruise at optimum altitude"
  at 8.5 units.
  ...
  2000-2500 at 30,000MSL at 0.60M. At optimum cruising speed and altitude, we
  should be able to attain over 6 hours flight time, far beyond any regular
  mission duration.
  ...
  for the Tomcat over 15000 feet ASL, impacting only speed. Its stores layouts
  ...
  climbs, not letting the AoA hit over 10 so the Manoeuvring flaps kick in and
  the speed is hampered, and so on. We are also unlikely to get a benefit on the
  long loiter times either, due to combat performance demanding use of
